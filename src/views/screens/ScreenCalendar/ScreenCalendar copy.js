...
export default function ScreenCalendar(props) {
    ...
    // boocking conditions
    const minimunPrice = 600;
    const checkBoockingConditions = (price, dayRange) => {
        if (price < minimunPrice) {
            return [false, `Precio mínimo no alcanzado, la reserva debe ser de al menos ${minimunPrice}€.`]
        }
        return [true, ''];
    }
    ...
    *****************************/
    ...
    function setRange(firstDateObject, secondDateObject) {
        ...
        const checkedBoockingConditions = checkBoockingConditions(totalPrice, null);
        if (checkedBoockingConditions[0] === false) {
            unselectAllCells();
            setAlertWithTimer({
                'display': true, 'severity': 'warning',
                'message': checkedBoockingConditions[1],
            })
            return
        }
